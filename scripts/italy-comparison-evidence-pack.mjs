...
function buildCandidate({ provider, offerKey, fieldName, rawValue, normalizedValue, evidence, observedAt, warnings = [] }) {
  ...
  const candidate = {
    ...
    subjectKey: offerKey,
    fieldName,
    scope: Object.freeze({ provider, destination: 'italy', scenarioId: SCENARIO.id, deviceModel: null, deviceRegion: null }),
    rawValue,
    normalizedValue,
...
const observedField = () => fieldState('observed');
const partialField = (reason) => fieldState('partial', reason);
const unknownField = (reason) => fieldState('unknown', reason);
const notApplicableField = (reason) => fieldState('not_applicable', reason);
...
export function extractAiralo(snapshots) {
  ...
  candidates.push(buildCandidate({ provider: 'airalo', offerKey, fieldName: 'network', rawValue: network[1], normalizedValue: { operators: [network[1]], completeness: 'partial', additionalOperatorsUnresolved: 2 }, evidence: [evidenceRef(plan, locatorFromMatch(plan, network, network[1]))], observedAt, warnings: ['expanded_network_list_not_captured'] }));
...
function extractHolafly(snapshots) {
  ...
  candidates.push(buildCandidate({ provider: 'holafly', offerKey, fieldName: 'destination_coverage', rawValue: scope[0], normalizedValue: { countries: ['IT'], scope: 'local' }, evidence: [evidenceRef(plan, locatorFromMatch(plan, scope, scope[0]))], observedAt }));
  coverage.destination_coverage = observedField();

  const row = findMatch(plan.visibleText, /(10\s+giorni)\s+(\d+(?:[.,]\d{1,2})\s*€\s*EUR)/i, 'Holafly 10-day price row');
  candidates.push(buildCandidate({ provider: 'holafly', offerKey, fieldName: 'validity_days', rawValue: row[1], normalizedValue: { duration: 10, unit: 'day' }, evidence: [evidenceRef(plan, locatorFromMatch(plan, row, row[1]))], observedAt }));
  candidates.push(buildCandidate({ provider: 'holafly', offerKey, fieldName: 'price', rawValue: row[2], normalizedValue: parseEur(row[2]), evidence: [evidenceRef(plan, locatorFromMatch(plan, row, row[2]))], observedAt, warnings: ['source_currency_preserved'] }));
  coverage.validity_days = observedField();
  coverage.price = observedField();
  ...
  candidates.push(buildCandidate({ provider: 'holafly', offerKey, fieldName: 'network', rawValue: network[0], normalizedValue: { operators: ['Vodafone Italy', 'WINDTRE'], completeness: 'declared' }, evidence: [evidenceRef(plan, locatorFromMatch(plan, network, network[0]))], observedAt }));
...
function extractUbigi(snapshots) {
  ...
  for (const baseCandidate of base.candidates) {
    candidates.push(buildCandidate({
      provider: 'ubigi',
      offerKey,
      fieldName: baseCandidate.fieldName,
      rawValue: baseCandidate.rawValue,
      normalizedValue: baseCandidate.normalizedValue,
      evidence: [evidenceRef(plan, Object.freeze({ ...baseCandidate.evidenceLocator, sourceKey: plan.sourceKey, snapshotId: plan.snapshotId }))],
      observedAt: plan.fetchedAt,
      warnings: baseCandidate.warnings,
    }));
    coverage[baseCandidate.fieldName] = observedField();
  }
...
export function packSemanticProjection(pack) {
  return Object.freeze({ scenario: pack.scenario, offers: pack.offers.map(semanticOfferProjection).sort((a, b) => a.provider.localeCompare(b.provider)), ranking: pack.ranking });
}
...
async function fetchWithRedirects(source, fetchImpl) {
  ...
  for (let index = 0; index <= MAX_REDIRECTS; index += 1) {
    const response = await fetchImpl(current, { method: 'GET', redirect: 'manual', headers: { accept: 'text/html,application/xhtml+xml;q=0.9', 'user-agent': 'SenzaRoamingEvidencePackSpike/1.0 (+https://senzaroaming.it/metodo)' } });
...
export async function captureSource(source, { fetchImpl = fetch, now = () => new Date() } = {}) {
  ...
  return Object.freeze({
    snapshot: buildSourceSnapshot({ sourceKey: source.key, requestedUrl: source.url, finalUrl, redirectChain, fetchedAt, httpStatus: response.status, contentType: response.headers.get('content-type') || '', etag: response.headers.get('etag'), lastModified: response.headers.get('last-modified'), body }),
    body,
  });
}
...
async function main() {
  ...
  for (const offer of artifact.pack.offers) {
    ...
    for (const [fieldName, state] of Object.entries(offer.coverage)) if (state.state !== 'observed') console.log(`  ${fieldName}: ${state.state}${state.reason ? ` — ${state.reason}` : ''}`);
  }
