...
**Conseguenza:** M5.5b.2 ha aggiunto home, listing, trust, articolo e 404 Astro, parametrizzato i componenti con render mode `preview | canonical` e verificato canonical, cache, robots, JSON-LD, internal link, published-only, empty state e fail-closed. Lo smoke `smoke:public-canonical-astro` costruisce un wrapper locale limitato ai route kind `canonical-static`, `canonical-article` e `public-404`, poi elimina wrapper, configurazione e stato D1. La CI finale #350 ha verificato anche tutte le preview, sitemap/robots legacy, API, provider redirect e Control Room. Il default resta `createPublicWorker(activePublicRouteDecision)` con `activePublicRouteDecision = currentPublicRouteDecision`; quindi il runtime live continua a usare il backend legacy per le route canoniche. Sitemap, robots, `/go/*`, `/api/*`, D1, publication gate, analytics e affiliazioni restano invariati. Il cutover resta una PR M5.7 separata.

## ADR-030 — Sitemap e robots condivisi prima del trasferimento di ownership

**Stato:** accettata come scope; implementazione non ancora avviata

**Decisione:** estrarre query, validazione, serializzazione XML e documento robots in un modulo server-only condiviso tra backend legacy e Astro. Compilare gli endpoint Astro `/sitemap.xml` e `/robots.txt`, ma testarli soltanto tramite un wrapper locale che assegna ad Astro il route kind `seo-endpoint`; il deploy normale continua a usare la current matrix.

**Razionale:** oggi la sitemap mantiene in `src/pages.ts` una lista statica duplicata, la query D1 e la concatenazione XML, mentre robots è una stringa inline in `src/index.ts`. Copiare questi handler in Astro produrrebbe una terza fonte di verità e permetterebbe drift su URL, ordine, `lastmod`, esclusioni e direttive crawler. Un builder condiviso rende il contratto verificabile prima del cutover e conserva la possibilità di rollback senza migrazioni.

**Conseguenza:** la branch `feat/public-seo-endpoint-parity`, autorizzata da `docs/PUBLIC-SEO-ENDPOINT-PARITY-SCOPE.md`, dovrà derivare le route statiche da `PUBLIC_CANONICAL_STATIC_PATHS`, leggere soltanto pagine `published`, validare slug e date, produrre XML e robots deterministici e fallire senza documento parziale su righe pubblicate invalide. Legacy e Astro useranno lo stesso contratto; populated, empty e invalid state saranno confrontati in `workerd`. `activePublicRouteDecision` non cambia; sitemap e robots live restano backend-owned. Non vengono introdotti sitemap index, Search Console submission, analytics, affiliazioni, D1 mutation, publication capability o cutover.
