...
**Conseguenza:** M5.5b.2 ha aggiunto home, listing, trust, articolo e 404 Astro, parametrizzato i componenti con render mode `preview | canonical` e verificato canonical, cache, robots, JSON-LD, internal link, published-only, empty state e fail-closed. Lo smoke `smoke:public-canonical-astro` costruisce un wrapper locale limitato ai route kind `canonical-static`, `canonical-article` e `public-404`, poi elimina wrapper, configurazione e stato D1. La CI #345 ha verificato anche tutte le preview, sitemap/robots legacy, API, provider redirect e Control Room. Il default resta `createPublicWorker(activePublicRouteDecision)` con `activePublicRouteDecision = currentPublicRouteDecision`; quindi il runtime live continua a usare il backend legacy per le route canoniche. Sitemap, robots, `/go/*`, `/api/*`, D1, publication gate, analytics e affiliazioni restano invariati. Il cutover resta una PR M5.7 separata.
