...
<meta name="robots" content="noindex,nofollow"><title>Senza Roaming Control Room</title>
<style>
:root{color-scheme:dark;--bg:#0b0d10;--panel:#13171c;--line:#28303a;--text:#f3f5f7;--muted:#9da8b5;--accent:#ffcc66;--good:#69d391;--bad:#ff7a7a;--blue:#79b8ff}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font:14px/1.5 Inter,ui-sans-serif,system-ui,sans-serif}header{position:sticky;top:0;z-index:3;background:rgba(11,13,16,.95);border-bottom:1px solid var(--line);padding:16px 22px;backdrop-filter:blur(12px)}h1,h2,h3,p{margin-top:0}.top{display:flex;gap:12px;align-items:center;flex-wrap:wrap}.brand{font-size:19px;font-weight:800;margin-right:auto}.status{color:var(--muted)}main{max-width:1500px;margin:auto;padding:22px}.grid{display:grid;gap:14px}.cards{grid-template-columns:repeat(auto-fit,minmax(150px,1fr));margin-bottom:18px}.card,.panel{background:var(--panel);border:1px solid var(--line);border-radius:14px;padding:15px}.metric{font-size:27px;font-weight:800}.label,.muted{color:var(--muted)}.panels{grid-template-columns:repeat(auto-fit,minmax(420px,1fr))}.wide{grid-column:1/-1}button,input,select,textarea{font:inherit;color:var(--text);background:#0d1116;border:1px solid var(--line);border-radius:8px;padding:9px 11px}button{cursor:pointer;background:#1c2530}button.primary{background:#5b4315;border-color:#9e7629;color:#fff2cf}button.danger{background:#3a1919}input,textarea,select{width:100%}textarea{min-height:76px;resize:vertical}.formgrid{display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:9px}.actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:10px}.tablewrap{overflow:auto;max-height:460px;border:1px solid var(--line);border-radius:10px}table{border-collapse:collapse;width:100%;min-width:760px}th,td{text-align:left;padding:9px;border-bottom:1px solid var(--line);vertical-align:top}th{position:sticky;top:0;background:#171c22}.pill{display:inline-block;padding:2px 7px;border:1px solid var(--line);border-radius:99px;font-size:12px}.verified,.approved,.completed{color:var(--good)}.failed,.contradicted{color:var(--bad)}.pending,.review,.proposed{color:var(--accent)}a{color:var(--blue)}pre{white-space:pre-wrap;word-break:break-word}.hidden{display:none}.notice{padding:10px;border-left:3px solid var(--accent);background:#17130b;margin-bottom:14px}.log{max-height:140px;overflow:auto;color:var(--muted);font-family:ui-monospace,monospace}
</style></head><body>
<header><div class="top"><div class="brand">Senza Roaming · Control Room</div><input id="token" type="password" autocomplete="off" placeholder="Maintenance token" style="max-width:280px"><button id="save" class="primary">Apri sessione</button><button id="lock">Blocca</button><button id="refresh">Aggiorna</button><span id="status" class="status">Sessione bloccata</span></div></header>
<main><div class="notice"><strong>Nessuna pubblicazione automatica.</strong> Le azioni di questa dashboard operano solo sui gate già presenti; un draft approvato resta una pagina in <code>review</code>.</div>
<section id="cards" class="grid cards"></section>
<section class="grid panels">
<div class="panel"><h2>Avvia ricerca recente</h2><label>Query, una per riga</label><textarea id="researchQueries">eSIM Cina problemi\neSIM viaggio VPN</textarea><div class="actions"><button data-action="research" class="primary">Avvia Workflow</button></div></div>
...
<div class="panel"><h2>Esito claim atomico</h2><div class="formgrid"><label>Candidate ID<input id="candidateId" type="number" min="1"></label><label>Esito<select id="claimOutcome"><option>verified</option><option>contradicted</option><option>insufficient</option><option>dismissed</option></select></label><label>Source kind<select id="sourceKind"><option value="official_provider">official_provider</option><option value="official_help">official_help</option><option value="official_terms">official_terms</option><option value="regulator">regulator</option><option value="manufacturer">manufacturer</option><option value="first_party_test">first_party_test</option></select></label><label>Freshness giorni<input id="freshnessDays" type="number" value="14" min="1"></label></div><label>URL fonte<input id="sourceUrl" placeholder="https://..."></label><label>Etichetta fonte<input id="sourceLabel"></label><label>Evidenza<textarea id="claimEvidence"></textarea></label><label>Note<textarea id="claimNotes"></textarea></label><div class="actions"><button data-action="claimResult" class="primary">Registra esito</button></div></div>
<div class="panel wide"><h2>Pipeline editoriale</h2><div id="briefs" class="tablewrap"></div></div>
<div class="panel wide"><h2>Claim atomici</h2><div id="claims" class="tablewrap"></div></div>
<div class="panel"><h2>Coda operativa</h2><div id="queue" class="tablewrap"></div></div>
<div class="panel"><h2>Evidence bundle e draft</h2><div id="bundles" class="tablewrap"></div></div>
<div class="panel wide"><h2>Audit recente</h2><div id="audit" class="tablewrap"></div></div>
<div class="panel wide"><h2>Console</h2><pre id="log" class="log"></pre></div>
</section></main>
<script>
(function(){
var key='srMaintenanceToken',token=sessionStorage.getItem(key)||'',data=null;
var $=function(id){return document.getElementById(id)}, esc=function(v){return String(v==null?'':v).replace(/[&<>"']/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]})};
$('token').value=token;
function setStatus(text,bad){$('status').textContent=text;$('status').style.color=bad?'var(--bad)':'var(--muted)'}
function log(value){$('log').textContent=(typeof value==='string'?value:JSON.stringify(value,null,2))+'\n'+$('log').textContent}
async function api(path,options){if(!token)throw new Error('Inserisci il maintenance token');var init=options||{};init.headers=Object.assign({'Authorization':'Bearer '+token,'Content-Type':'application/json'},init.headers||{});var r=await fetch(path,init),text=await r.text(),body;try{body=JSON.parse(text)}catch(e){body={raw:text}}if(!r.ok)throw new Error(body.error||('HTTP '+r.status));return body}
function pill(v){return '<span class="pill '+esc(v)+'">'+esc(v)+'</span>'}
function table(headers,rows){return '<table><thead><tr>'+headers.map(function(h){return '<th>'+esc(h)+'</th>'}).join('')+'</tr></thead><tbody>'+rows.map(function(r){return '<tr>'+r.map(function(c){return '<td>'+c+'</td>'}).join('')+'</tr>'}).join('')+'</tbody></table>'}
function render(d){data=d;var o=d.overview,c=[['Fonti',o.sources_total],['Fonti scadute',o.sources_due],['Queue pending',o.queue_pending],['Segnali eligible',o.signals_eligible],['Brief proposti',o.briefs_proposed],['Claim verificati',o.claims_verified],['Claim pending',o.claims_pending],['Draft approved',o.drafts_approved],['Pagine review',o.pages_review],['Pagine pubblicate',o.pages_published]];$('cards').innerHTML=c.map(function(x){return '<div class="card"><div class="label">'+esc(x[0])+'</div><div class="metric">'+esc(x[1])+'</div></div>'}).join('');
$('briefs').innerHTML=table(['ID','Titolo','Score','Stato','Bundle','Readiness','Draft'],d.briefs.map(function(b){return [esc(b.id),'<strong>'+esc(b.proposed_title)+'</strong><br><span class="muted">'+esc(b.slug_suggestion)+'</span>',esc(b.priority_score),pill(b.status),esc(b.bundle_id||'—'),esc(b.readiness_score||'—')+' '+(b.readiness_status?pill(b.readiness_status):''),esc(b.draft_id||'—')+' '+(b.draft_status?pill(b.draft_status):'')]}));
$('claims').innerHTML=table(['ID','Brief','Soggetto','Claim','Stato','Fonte','Scadenza','Task'],d.claims.map(function(c){return [esc(c.id),esc(c.brief_id),esc(c.subject_key),esc(c.claim_text),pill(c.status),c.source_url?'<a target="_blank" rel="noopener" href="'+esc(c.source_url)+'">'+esc(c.source_label||c.source_kind)+'</a>':'—',esc(c.valid_until||'—'),c.task_status?pill(c.task_status)+' #'+esc(c.task_id):'—']}));
$('queue').innerHTML=table(['ID','Tipo','Entità','Priorità','Stato','Errore'],d.queue.map(function(q){return [esc(q.id),esc(q.task_type),esc(q.entity_key),esc(q.priority),pill(q.status),esc(q.last_error||'')]}));
$('bundles').innerHTML=table(['Tipo','ID','Slug','Versione','Score/Stato'],d.evidenceBundles.map(function(b){return ['bundle',esc(b.id),esc(b.page_slug),esc(b.version),esc(b.readiness_score)+' '+pill(b.review_status)]}).concat(d.drafts.map(function(x){return ['draft',esc(x.id),esc(x.page_slug),esc(x.version),pill(x.status)+' '+esc(x.prompt_version)]})));
$('audit').innerHTML=table(['Quando','Dominio','Azione','Attore','Entità'],d.audit.map(function(a){return [esc(a.created_at),esc(a.domain),pill(a.action),esc(a.actor),esc(a.entity)]}));setStatus('Aggiornato '+new Date(d.generatedAt).toLocaleString(),false)}
async function load(){try{setStatus('Caricamento…');render(await api('/api/maintenance/control-room'))}catch(e){setStatus(e.message,true);log(e.message)}}
function val(id){return $(id).value.trim()}function intVal(id){return Number(val(id)||0)}
async function post(path,body){var result=await api(path,{method:'POST',body:JSON.stringify(body)});log(result);await load();return result}
async function act(name){try{if(name==='research'){var qs=val('researchQueries').split(/\n+/).map(function(x){return x.trim()}).filter(Boolean);await post('/api/maintenance/research-run',{queries:qs,reason:'control_room_manual'})}
else if(name==='acceptBrief')await post('/api/maintenance/editorial-brief-action',{briefIds:[intVal('briefId')],status:'accepted',notes:'Accettato dalla Control Room; nessun claim è pubblicabile senza verifica.'});
else if(name==='convertBrief')await post('/api/maintenance/editorial-brief-convert',{briefId:intVal('briefId'),actor:'christian'});
else if(name==='evaluate')await post('/api/maintenance/page-readiness-evaluate',{briefId:intVal('briefId'),actor:'christian'});
else if(name==='approveBundle')await post('/api/maintenance/page-readiness-action',{bundleId:intVal('bundleId'),action:'approved_for_draft',actor:'christian',notes:'Approvato dalla Control Room esclusivamente per la generazione di un draft in review.'});
else if(name==='generateDraft')await post('/api/maintenance/editorial-draft-generate',{bundleId:intVal('bundleId'),actor:'christian'});
else if(name==='approveDraft')await post('/api/maintenance/editorial-draft-action',{draftId:intVal('draftId'),action:'approved',actor:'christian',notes:val('draftNotes')});
else if(name==='changesDraft')await post('/api/maintenance/editorial-draft-action',{draftId:intVal('draftId'),action:'changes_requested',actor:'christian',notes:val('draftNotes')});
else if(name==='previewDraft'){var html=await api('/api/maintenance/editorial-draft-preview?draftId='+intVal('draftId'),{headers:{'Accept':'text/html'}});log(html)}
else if(name==='claimResult'){var outcome=val('claimOutcome'),body={candidateId:intVal('candidateId'),outcome:outcome,actor:'christian',evidence:val('claimEvidence'),notes:val('claimNotes')};if(outcome==='verified'||outcome==='contradicted'){body.source={kind:val('sourceKind'),label:val('sourceLabel'),url:val('sourceUrl'),trustLevel:5,freshnessDays:intVal('freshnessDays')};body.value={recordedBy:'control-room'};body.confidence=1}await post('/api/maintenance/editorial-atomic-claim-result',body)}}catch(e){setStatus(e.message,true);log(e.message)}}
$('save').onclick=function(){token=val('token');sessionStorage.setItem(key,token);load()};$('lock').onclick=function(){token='';sessionStorage.removeItem(key);$('token').value='';setStatus('Sessione bloccata')};$('refresh').onclick=load;document.querySelectorAll('[data-action]').forEach(function(b){b.onclick=function(){act(b.getAttribute('data-action'))}});if(token)load();
})();
</script></body></html>`;
