...
function promptFor(bundle: Obj, verifiedClaims: Obj[], excludedClaimIds: number[]): string {
  ...
  return [
    ...
    '- title, metaDescription, h1, directAnswer e intro devono avere fieldClaimIds non vuoti e realmente pertinenti.',
    '- directAnswer e intro devono nominare esplicitamente ogni provider cui attribuiscono una dichiarazione.',
...
function fieldMentionsSubjects(text: string, claimIds: number[], claimMap: Map<number, Obj>): boolean {
  ...
  const subjects = [...new Set(claimIds.map((id) => claimMap.get(id))
    .filter((claim): claim is Obj => Boolean(claim) && claim.subjectType === 'provider')
    .map((claim) => normalizedSubject(claim.subjectKey)).filter(Boolean))];
...
function normalizeDraft(data: Obj, verifiedClaims: Obj[]): Normalized | null {
  ...
  return {
    title, metaDescription, eyebrow, h1, directAnswer, intro, primaryKeyword,
    fieldClaimIds, blocks, faq, sectionClaims, faqClaims, usedClaimIds: [...used]
  };
}
...
function deterministicWarnings(blocks: ContentBlock[], bundle: Obj, excluded: Obj[]): ContentBlock[] {
  const result = [...blocks];
  for (const conflict of arr(bundle.conflicts).map(obj).filter((item): item is Obj => Boolean(item))) {
    result.push({
      type: 'callout',
...
async function draftResponse(env: Env, draftId: number): Promise<Obj | null> {
  ...
  return {
    id: Number(row.id), evidenceBundleId: Number(row.evidence_bundle_id), draftKey: row.draft_key,
    version: Number(row.version), pageSlug: row.page_slug, pageType: row.page_type,
    model: row.model, promptVersion: row.prompt_version, status: row.status,
    page: {
      title: row.title, metaDescription: row.meta_description, eyebrow: row.eyebrow, h1: row.h1,
      directAnswer: row.direct_answer, intro: row.intro,
      content: parseJson(row.content_json, []), faq: parseJson(row.faq_json, []),
    ...
    responseId: row.response_id || null, usage: parseJson(row.usage_json, {}),
    error: row.error_message || null, generatedBy: row.generated_by,
    reviewedBy: row.reviewed_by || null, reviewedAt: row.reviewed_at || null,
    reviewNotes: row.review_notes || '', materializedPageStatus: row.page_status || null,
...
async function generateGroundedDraft(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    `).bind(
      ...
      'editorial_review', 'page', pageSlug,
    ...
    if (error instanceof VertexGatewayError) {
      return json({ ok: false, error: 'ai_gateway_upstream_error', upstreamStatus: error.upstreamStatus, detail: error.detail, draftId }, 502);
    }
